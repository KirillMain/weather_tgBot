...
bot = telebot.TeleBot('6361012885:AAFi7sHk2JtETkPSvov3tOJVDFiZtvwZGeo')
...
def send_welcome(message):
    ...
    with open("data/chooses/users_params.txt", "w", encoding="utf-8") as f:
        f.write(str(d))
    st = open("stick/hello.tgs", 'rb')
...
def times(message):
    ...
    if idd not in arr:
        arr.append(idd)
        text = f"Время {fake_t} было добавлено в рассылку."
    else:
        arr.remove(idd)
        text = f"Время {fake_t} было убрано из рассылки."

    with open('data/chooses/user_times.txt', 'w', encoding='utf-8') as f:
        f.write(str(d))
